...
If you want to see the module in action, then check out the sample application which runs with Spring Boot and is super easy to setup: [expenses-control](https://github.com/guylabs/expenses-control)
...
The `_embedded` property holds an array of the requested transactions, and each of the transactions has again a `_links` property which defines the resources for the specific item. You can find more about how the responses of *Spring Data REST* look like [here](http://projects.spring.io/spring-data-rest).

This *Angular* module provides two ways of processing a response from the *Spring Data REST* backend and ease the usage with the resources and embedded transactions:
...
2. `_embeddedItems`: this property replaces the `_embedded` property and sets the named array (`categories` in the upper example response) with the embedded transactions as its value. Read more about this property [here](#usage-of-_embeddedItems-property).
...
Please read on on how to use the `_resources` method and the `_embeddedItems` property to ease the handling of resources and embedded transactions.
...
The `_embeddedItems` property is just a convention property created by the `SpringDataRestAdapter` to easily iterate over the `_emebedded` transactions in the response. Like with the `_resources` method, the `SpringDataRestAdapter` will recursively create an `_embeddedItems` property on the same level as a `_embedded` property exists for all the JSON response properties.
...
* `embeddedKey` (default: `_embedded`): the property name where the embedded transactions are stored.
* `embeddedNewKey` (default: `_embeddedItems`): the property name where the array of embedded transactions are stored.
